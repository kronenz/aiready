    ...
    <div className="w-80 border-l flex flex-col overflow-hidden shrink-0"
      ...
      <div className="h-10 flex items-center justify-between px-4 border-b shrink-0"
        ...
        <button onClick={onClose}
          className="p-1 rounded transition-colors hover:bg-[#1e293b]"
          style={{ color: '#64748b' }}>
          <X size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {mode === 'component' && selectedComponent && <ComponentDetail component={selectedComponent} />}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <div className="text-xs mt-0.5" style={{ fontFamily: "'JetBrains Mono', monospace", color: component.color }}>{component.version}</div>
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-2xl font-bold" style={{ fontFamily: "'JetBrains Mono', monospace", color: '#e2e8f0' }}>{component.healthScore}</span>
        ...
        <div className="space-y-2">
          <div>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span style={{ fontFamily: "'JetBrains Mono', monospace", color: '#e2e8f0' }}>{component.cpu}%</span>
            </div>
          ...
          <div>
            <div className="flex justify-between text-xs mb-1">
              ...
              <span style={{ fontFamily: "'JetBrains Mono', monospace", color: '#e2e8f0' }}>{component.mem}%</span>
            </div>
      ...
      <div className="flex gap-2 pt-2">
        {['📋 로그', '⚙️ 설정', '🤖 Agent'].map(label => (
          <button key={label}
            className="flex-1 px-2 py-1.5 rounded text-xs font-medium border transition-colors hover:border-[#334155]"
...
function AlertList() {
  const severityColor: Record<string, string> = { critical: '#ef4444', warning: '#eab308', info: '#64748b' };

  return (
    <div className="space-y-2">
      {alerts.map(alert => (
        <div key={alert.id} className="rounded overflow-hidden" style={{ background: '#0f172a' }}>
          <div className="flex" style={{ borderLeft: `3px solid ${severityColor[alert.severity]}` }}>
            <div className="flex-1 p-3">
              <div className="flex items-center gap-2 mb-1">
                <span className="px-1.5 py-0.5 rounded text-xs font-medium"
                  style={{ background: `${severityColor[alert.severity]}20`, color: severityColor[alert.severity] }}>
                  {alert.severity}
                </span>
                <span className="text-xs" style={{ fontFamily: "'JetBrains Mono', monospace", color: '#64748b' }}>{alert.time}</span>
              ...
              {alert.status === 'firing' && (
                <button className="mt-2 px-2 py-1 rounded text-xs font-medium transition-colors hover:bg-[#7c3aed30]"
